...
const TicketSystem = () => {
  ...
  const [staffModal, setStaffModal] = useState({
    open: false,
    type: null, // "ASSIGN" | "TRANSFER"
    ticket: null,
  });

  const [staffIdInput, setStaffIdInput] = useState("");

  const isAdmin = Boolean(localStorage.getItem("mm_admin_token"));
  ...
  // To close the Admin Action Button on each Ticket List Overview
  useEffect(() => {
    const closeMenu = () => setOpenActionTicketId(null);
  ...
  const fetchTickets = async () => {
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/tickets/admin/all?${params}`,
        {
          headers: {
  ...
  const handleAssignToStaff = async () => {
    if (!staffModal.ticket || !staffIdInput) return;
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/tickets/admin/${staffModal.ticket.id}/assign`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setSelectedTicket((prev) =>
        prev ? { ...prev, assigned_staff: staffIdInput } : prev,
      );

      // Update ticket list
      setTickets((prev) =>
        prev.map((t) =>
          t.id === staffModal.ticket.id
            ? { ...t, staff_name: staffIdInput }
            : t,
        ),
      );

      setStaffModal({ open: false, type: null, ticket: null });
      setStaffIdInput("");
    } catch (error) {
      console.error("Assign to staff failed:", error);
    }
  };
  ...
  const handleTransferToStaff = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/tickets/admin/${staffModal.ticket.id}/transfer`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          body: JSON.stringify({ to_staff_id: staffIdInput }),
        },
      );

      if (!response.ok) {
        throw new Error("Failed to transfer staff");
      }
      ...
      console.error("Transfer to staff failed:", error);
    }
  };
                        ...
                        <div className="flex items-start justify-between">
                          ...
                          {isAdmin && (
                            <div className="relative">
                              ...
                              {openActionTicketId === ticket.id && (
                                <div className="absolute right-full top-0 mr-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          ...
          >
            <h3 className="text-lg font-semibold text-gray-800 mb-2">
              {staffModal.type === "ASSIGN"
                ? "Assign Ticket to Staff"
                : "Transfer Ticket to Staff"}
            </h3>

            {/* Description */}
            <p className="text-sm text-gray-600 mb-4">
              {staffModal.type === "ASSIGN" &&
                (staffModal.ticket?.staff_name &&
                staffModal.ticket.staff_name !== "Unassigned" ? (
                  <>
                    The staff is already assigned, you can transfer it to
                    someone else only.
                  </>
                ) : (
                  <>The selected ticket will be assigned to your given staff.</>
                ))}

              {staffModal.type === "TRANSFER" &&
                (staffModal.ticket?.staff_name &&
                staffModal.ticket.staff_name !== "Unassigned" ? (
                  <>
                    You selected ticket will be transferred from{" "}
                    <span className="font-medium">
                      {staffModal.ticket.staff_name}
                    </span>{" "}
                    to your given staff.
                  ...
                  <>
                    There is no assigned staff. First assign a staff to transfer
                    it to someone else.
                  </>
                ))}
            </p>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() => {
                  if (staffModal.type === "ASSIGN") {
                    handleAssignToStaff();
                  } else if (staffModal.type === "TRANSFER") {
                    handleTransferToStaff();
                  }
                }}
                disabled={
                  !staffIdInput ||
